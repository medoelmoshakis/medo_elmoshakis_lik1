...
st.markdown("""
    ...
    div[data-testid="stMetricValue"] { color: #FFD700; text-align: center; font-family: monospace; }
    </style>
...
# حقل الإدخال والزر المعدل بناءً على طلبك
        ...
        class AppRunner:
            ...
            async def start_worker(self, session, vid):
                while True:
                    ...
                    await asyncio.sleep(0.01) # لمنع تعليق المتصفح
                ...
                async with aiohttp.ClientSession() as session:
                    try:
                        async with session.get(self.url, allow_redirects=True) as r:
                            ...
                            else: st.error("رابط فيديو غير صحيح!")
                    except: st.error("حدث خطأ في الاتصال")
